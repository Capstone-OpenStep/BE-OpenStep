...
public class Member extends BaseEntity {
    ...
    public List<MemberAchievement> getUnLockedAchievements(AcheivementType type) {
        return achievements.stream()
            .filter(MemberAchievement::isUnlocked)
            .toList();
    }

    public Optional<MemberAchievement> getAcheivmentByType(AcheivementType type) {
        return achievements.stream()
            .filter(achievement -> achievement.getType() == type)
            .findFirst();
    }

    public boolean hasUnlockedAchievement(AcheivementType type) {
        return getAcheivmentByType(type)
